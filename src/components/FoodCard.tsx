...
const FoodCard = ({ item, onClick }: FoodCardProps) => {
  return (
    <div
      className="w-[48%] h-52 sm:h-60 bg-darkGreen  rounded-xl shadow-xl  hover:scale-[1.02] transition-all ease-in-out duration-[500] flex flex-col items-center justify-start cursor-pointer pt-2"
    ...
    >
      ...
      <div className="w-full h-[40%] flex justify-center items-center px-2 py-2">
        <div className=" w-full h-full bg-cream rounded-md text-sm font-medium font-archivo px-2 pt-2 flex flex-col justify-start items-start">
          <span className="font-bold text-[10px] text-nowrap">{item.data.name}</span>
          <span className="hidden sm:block">
            prep time: {item.data.cookTime}
          </span>
